...
########################################################################
# Global settings:
SIZE = 580  # size of display window for single instance
STATUS_HEIGHT = 80  # height of status bar in display window
STATUS_HEIGHT2 = 30  # height of status bar within instance subwindows
STATUS_HEIGHT3 = 2  # height of status bar at the bottom (Github info)
DELIM_WIDTH = 0 # width of delimiter between direct and sim. ann. output
...
def change_N(N):
    ...
    return N, dcities, scities, iters, siters, diters, d_energy_min, s_energy_min, beta, n_accept, timeit.default_timer(), {
        "direct": {"iters": [], "lengths": []}, "simann": {"iters": [], "lengths": []}}
...
def get_filename(N, iters):
    ...
    filename = "Images/TSP" + str(N) + "_iters" + str(iters) + "_" + \
...
pygame.display.set_caption("TSP: simulated annealing")
...
def mainloop(surface, N, dcities, scities, start_timer):
   ...
   # button_save = Button(60, 30, "Save", COLORS["LAVENDER"], COLORS["BLACK"])
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                if button_ncity_10.IsPressed(pygame.mouse.get_pos()):
                    # N = 10 selected
                    N, dcities, scities, iters, siters, diters, d_energy_min, s_energy_min, beta, n_accept, start, plot_data = change_N(
                        10)
        ...
        new_scities, beta, n_accept, s_energy_min, new_route = simulated_annealing(N, scities[:], beta, n_accept,
                                                                                   s_energy_min)
        if new_route:
            if VERBOSE:
        ...
        if change:
            ...
            surface.fill(COLORS["PINK"], (0, 0, 2 * SIZE + DELIM_WIDTH, STATUS_HEIGHT))
            ...
            surface.fill(COLORS["DARKBLUE"],
                         (0, SIZE + STATUS_HEIGHT + STATUS_HEIGHT2, 2 * SIZE + DELIM_WIDTH, DELIM_WIDTH))
 ...
 # draw cities and roads:
            ...
            draw_text(surface, helv24, "Min. Tour Length:", (SIZE + DELIM_WIDTH + 260, STATUS_HEIGHT + 10),
                      COLORS["BLACK"])
